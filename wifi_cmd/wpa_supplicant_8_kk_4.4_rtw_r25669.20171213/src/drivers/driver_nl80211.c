...
//#define driver_nl80211_log(fmt, args...) printf(fmt"[ZWX][DRIVER_NL80211][FUNC:%s][LINE:%d] "fmt"\n", __FUNCTION__, __LINE__, ##args)
#define driver_nl80211_log(fmt, args...)

struct i802_bss;

struct i802_bss *thread_priv = NULL;

struct wpa_driver_scan_params * thread_scan_params = NULL;
...
int in_eloop_register_scan = 0;

static int driver_nl80211_connect = 0;

void *sched_scan_thread(void *data);
...
struct nl80211_global {
	struct dl_list interfaces;      //该链接包含 wpa_driver_nl80211_data 对象
	int if_add_ifindex;
	u64 if_add_wdevid;
	int if_add_wdevid_set;
	struct netlink_data *netlink;
	struct nl_cb *nl_cb;            //netlink 回调对象
	struct nl_handle *nl;           //nl_handle 的真实类型就是 libnl 定义的 nl_socket 。 nl成员变量用于发送 netlink event
	int nl80211_id;                 //nl80211 模块的 netlink family 值
	int ioctl_sock; /* socket for ioctl() use 用于 ioctl 函数的 socket 句柄*/

	struct nl_handle *nl_event;     //用于接收 netlink event
};
...
struct i802_bss {
	...
	struct i802_bss *next;  //指向下一个 i802_bss
	int ifindex;    //网络设备编号
	u64 wdev_id;
	...
	u8 addr[ETH_ALEN];  //用于存储 MAC 地址

	int freq;
	int if_dynamic;

	void *ctx;
	struct nl_handle *nl_preq, *nl_mgmt;        //用于接收 MLME 相关的 netlink 消息
	struct nl_cb *nl_cb;                        //netlink 回调对象
...
struct wpa_driver_nl80211_data {
	struct nl80211_global *global;          //指向 nl80211_global 对象
	struct dl_list list;
	struct dl_list wiphy_list;
	char phyname[32];
	void *ctx;                              //指向一个 wpa_supplicant 对象
	int ifindex;                            //对应网卡设备的索引
	int if_removed;
	int if_disabled;
	int ignore_if_down_event;
	struct rfkill_data *rfkill;             //rfkill 相关的上下文信息
	struct wpa_driver_capa capa;            //代表 kernel 中 wifi 驱动的 capability
	u8 *extended_capa, *extended_capa_mask;
	unsigned int extended_capa_len;
	int has_capability;

	int operstate;  //IfOperStatus 相关

	int scan_complete_events;

	struct nl_cb *nl_cb;        //libnl 回调对象
	...
	enum nl80211_iftype ap_scan_as_station; //接口类型
				 ...
				 struct nlattr **tb)
{
	...
	case NL80211_CMD_START_SCHED_SCAN:
		...
		driver_nl80211_connect = 0;
		//wpa_driver_nl80211_sched_scan(bss, NULL,thread_interval);
		//wpa_driver_nl80211_sched_scan();//(void *priv,struct wpa_driver_scan_params *params,u32 interval)
        //wpa_supplicant_req_scan(drv->ctx, 5, 0);
		//driver_nl80211_scan2
		break;
	case NL80211_CMD_SCHED_SCAN_STOPPED:
		wpa_dbg(drv->ctx, MSG_DEBUG, "nl80211: Sched scan stopped");
		driver_nl80211_connect = 1;
	...
	case NL80211_CMD_NEW_SCAN_RESULTS:
		...
		eloop_cancel_timeout(wpa_driver_nl80211_scan_timeout, drv,
				     drv->ctx);
		printf("[ZWX] NL80211_CMD_NEW_SCAN_RESULTS \n");
		send_scan_event(drv, 0, tb);
		in_eloop_register_scan = 0;
		//printf("[ZWX] NL80211_CMD_NEW_SCAN_RESULTS : SCANNING \n");
		//wpa_supplicant_req_scan(drv->ctx, 5, 0);
		break;
	case NL80211_CMD_SCHED_SCAN_RESULTS:
		wpa_dbg(drv->ctx, MSG_DEBUG,
			"nl80211: New sched scan results available");
		printf("[ZWX] NL80211_CMD_SCHED_SCAN_RESULTS \n");
		send_scan_event(drv, 0, tb);
		break;
	case NL80211_CMD_SCAN_ABORTED:
		...
		eloop_cancel_timeout(wpa_driver_nl80211_scan_timeout, drv,
				     drv->ctx);
		printf("[ZWX] NL80211_CMD_SCAN_ABORTED \n");
	...
	if (tb[NL80211_ATTR_IFINDEX]) {
		...
		for (bss = &drv->first_bss; bss; bss = bss->next)
			if (ifidx == -1 || ifidx == bss->ifindex) {
			    printf("[ZWX] process_drv_event : 1 \n");
		...
		for (bss = &drv->first_bss; bss; bss = bss->next) {
			if (bss->wdev_id_set && wdev_id == bss->wdev_id) {
			    printf("[ZWX] process_drv_event : 2 \n");
				do_process_drv_event(bss, gnlh->cmd, tb);
				return NL_SKIP;
			}
		}
		...
		for (bss = &drv->first_bss; bss; bss = bss->next) {
			if ((ifidx == -1 && !wdev_id_set) ||
			    ...
			    (wdev_id_set && bss->wdev_id_set &&
			     wdev_id == bss->wdev_id)) {
			    printf("[ZWX] process_global_event \n");
				do_process_drv_event(bss, gnlh->cmd, tb);
				return NL_SKIP;
			}
		}
					     ...
					     void *handle)
{
	...
	nl_recvmsgs(handle, cb);        //cb 是 bss->nl_cb
}
				       ...
				       struct wiphy_info_data *info)
{
	...
	if (feat & NL80211_PROTOCOL_FEATURE_SPLIT_WIPHY_DUMP)
        //构造一个 NL80211_CMD_GET_WIPHY 命令以获取设备信息
    ...
    //发送命令并等待回复，回复消息将由 wiphy_info_handler 函数处理
				      ...
				      void *global_priv)
{
	...
	drv->ctx = ctx;         //ctx 的真正类型是 wpa_supplicant
	bss = &drv->first_bss;
	bss->drv = drv;
	bss->ctx = ctx;
	...
	if (drv->global) {
        //把自己加入到 nl80211_global 中的 interfaces 链表中去
	    //////////////////////////////////////////////
...
static int nl80211_alloc_mgmt_handle(struct i802_bss *bss)
{
	...
	bss->nl_mgmt = nl_create_handle(drv->nl_cb, "mgmt");        //drv->nl_cb对应的回调对象是 process_drv_event
	if (bss->nl_mgmt == NULL)
		return -1;
...
wpa_driver_nl80211_finish_drv_init(struct wpa_driver_nl80211_data *drv)
{
	...
	drv->ifindex = if_nametoindex(bss->ifname);             //获取网卡设备的索引，属于 netdevice 编程范畴
	 ...
	 * with proper iftype.
	 */
    //设置接口类型； 该函数会调用到上一节提到的 nl80211_alloc_mgmt_handle
    ...
    //linux_set_iface_flags 通过 ioctl 方式启动 ifname 对应的网卡设备，该函数使用了 netdevice API
	if (linux_set_iface_flags(drv->global->ioctl_sock, bss->ifname, 1)) {
	    //启动设备失败时，要判断是不是 rfkill 禁止了该设备
		if (rfkill_is_blocked(drv->rfkill)) {
			wpa_printf(MSG_DEBUG, "nl80211: Could not yet enable "
				   "interface '%s' due to rfkill",
				   bss->ifname);
	        //如果是因为 rfkill 原因导致设备被禁止，则需要通知 wpa_supplicant
			drv->if_disabled = 1;       //该设备被 rfkill 禁止了
			send_rfkill_event = 1;      //该值表示需要设置 WPAS 的状态
		} else {
    ...
    //设置 Wi-Fi 设备的工作状态为 IF_OPER_DORMANT
	netlink_send_oper_ifla(drv->global->netlink, drv->ifindex,
			       1, IF_OPER_DORMANT);
#endif /* HOSTAPD */

    //通过 ioctl 方式获取指定网卡的MAC地址
	if (linux_get_ifhwaddr(drv->global->ioctl_sock, bss->ifname,
			       bss->addr))
		return -1;

	if (send_rfkill_event) {
        //添加一个超时任务，超时时间为 0 秒。超时处理函数为 wpa_driver_nl80211_send_rfkill， 该函数内部将设置 wpa_states 为 WPA_INTERFACE_DISABLED
...
{
    printf("wpa_driver_nl80211_scan_timeout function called \n");
	struct wpa_driver_nl80211_data *drv = eloop_ctx;
	if (drv->ap_scan_as_station != NL80211_IFTYPE_UNSPECIFIED) {
		wpa_driver_nl80211_set_mode(&drv->first_bss,
					    drv->ap_scan_as_station);
...
{
    driver_nl80211_log("----------");
    printf("[ZWX] nl80211_scan_common \n");
	struct nl_msg *msg;
	size_t i;
...
{
    driver_nl80211_log("----------");
    printf("[ZWX] wpa_driver_nl80211_scan \n");
	struct wpa_driver_nl80211_data *drv = bss->drv;
	int ret = -1, timeout;
...
static int wpa_driver_nl80211_sched_scan(void *priv,
					 ...
					 u32 interval)
{
    printf("[ZWX] wpa_driver_nl80211_sched_scan \n");
    driver_nl80211_log("----------");
	struct i802_bss *bss = priv;

	thread_priv = priv;
	thread_interval = interval;
	thread_scan_params = params;
	...
	if (!drv->capa.sched_scan_supported) {
	    printf("[ZWX] wpa_driver_nl80211_sched_scan : !drv->capa.sched_scan_supported \n");
	...
	msg = nl80211_scan_common(drv, NL80211_CMD_START_SCHED_SCAN, params,
				  bss->wdev_id_set ? &bss->wdev_id : NULL);
	if (!msg) {
	    printf("[ZWX] wpa_driver_nl80211_sched_scan : !msg \n");
		goto nla_put_failure;
	}
	...
	pthread_create(&tid, NULL,sched_scan_thread , NULL);


	printf("[ZWX] wpa_driver_nl80211_sched_scan : ret = %d \n",ret);
nla_put_failure:
	nlmsg_free(msg);
	return ret;
}
...
{
    printf("[ZWX] wpa_driver_nl80211_stop_sched_scan \n");
    driver_nl80211_log("----------");
	struct i802_bss *bss = priv;
	struct wpa_driver_nl80211_data *drv = bss->drv;
	int ret = 0;
...
{
    driver_nl80211_log("----------");
    printf("[ZWX] wpa_driver_nl80211_check_bss_status \n");
	size_t i;
...
{
    driver_nl80211_log("----------");
    printf("[ZWX] nl80211_get_scan_results \n");
	struct nl_msg *msg;
	struct wpa_scan_results *res;
	int ret;
	struct nl80211_bss_info_arg arg;
...
{
    driver_nl80211_log("----------");
    printf("[ZWX] wpa_driver_nl80211_get_scan_results \n");
	struct i802_bss *bss = priv;
	struct wpa_driver_nl80211_data *drv = bss->drv;
	struct wpa_scan_results *res;
...
{
    driver_nl80211_log("----------");
    printf("[ZWX] wpa_driver_nl80211_disconnect \n");
	int ret;
...
{
    driver_nl80211_log("----------");
    printf("[ZWX] wpa_driver_nl80211_authenticate \n");
	struct wpa_driver_nl80211_data *drv = bss->drv;
	int ret = -1, i;
	struct nl_msg *msg;
	...
	struct wpa_driver_associate_params *params)
{
    driver_nl80211_log("----------");
    driver_nl80211_connect = 1;
    printf("[ZWX] wpa_driver_nl80211_connect \n");
...
{
    driver_nl80211_log("----------");
    printf("[ZWX] wpa_driver_nl80211_associate \n");
	struct i802_bss *bss = priv;
	struct wpa_driver_nl80211_data *drv = bss->drv;
	int ret = -1;
	struct nl_msg *msg;
...
static void * nl80211_global_init(void)
{
	...
	cfg->ctx = global;
    //下面这三条语句用于创建 netlink socket 来接收来自内核的网卡状态变化事件(如 UP、DORMANT、REMOVED)，然后通过 eloop_register_read_sock 注册一个 netlink_recv 函数用于处理接收到的 socke 消息。
    //netlink_recv 函数内部将根据消息的类别来调用 netlink_cb 和 dellink_cb 以处理网卡状态变化事件。
    ...
    //nl80211 利用 netlink 机制和 wlan driver 交互
...
void *sched_scan_thread(void *data)
{
    printf("[ZWX] thread_start \n");
    while(main_run) {
        if (driver_nl80211_connect == 0) {
            if(in_eloop_register_scan == 0) {
                in_eloop_register_scan = 1;
                wpa_supplicant_req_scan(thread_priv->drv->ctx, thread_interval / 1000, 0);
            }
            //wpa_supplicant_trigger_scan(thread_priv->drv->ctx, thread_scan_params);
            //driver_nl80211_scan2(thread_priv, thread_scan_params);
            sleep(1);
        }
        else {
            sleep(thread_interval / 1000);
        }
    }
    printf("[ZWX] thread_quit \n");
    return NULL;
}
