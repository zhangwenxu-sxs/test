...
void wpas_p2p_continue_after_scan(struct wpa_supplicant *wpa_s)
{
	...
	if (!wpa_s->global->p2p_cb_on_scan_complete) {
	    printf("[ZWX] !wpa_s->global->p2p_cb_on_scan_complete \n");
	    return;
	}
	wpa_s->global->p2p_cb_on_scan_complete = 0;

	if (wpa_s->global->p2p_disabled || wpa_s->global->p2p == NULL) {
	    printf("[ZWX] wpa_s->global->p2p_disabled || wpa_s->global->p2p == NULL \n");
		return;
	}

	delay = wpas_p2p_search_delay(wpa_s);
	printf("[ZWX]delay: %d \n", delay);

	if (p2p_other_scan_completed(wpa_s->global->p2p) == 1) {
		wpa_dbg(wpa_s, MSG_DEBUG, "P2P: Pending P2P operation "
			"continued after successful connection");
		p2p_increase_search_delay(wpa_s->global->p2p, delay);
	}
}
