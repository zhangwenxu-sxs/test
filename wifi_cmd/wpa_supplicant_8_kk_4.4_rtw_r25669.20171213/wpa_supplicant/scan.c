...
wpa_supplicant_delayed_sched_scan_timeout(void *eloop_ctx, void *timeout_ctx)
{
	...
	printf("[ZWX] wpa_supplicant_delayed_sched_scan_timeout \n");
...
//	wpa_supplicant_notify_scanning(wpa_s, 1);
	printf("[ZWX] wpa_supplicant_start_sched_scan : interval = %d \n", interval);
...
static void wpa_setband_scan_freqs(struct wpa_supplicant *wpa_s,
				   struct wpa_driver_scan_params *params)
{
    printf("[ZWX] wpa_setband_scan_freqs \n");
...
{
    printf("[ZWX] wpa_supplicant_scan \n");
	struct wpa_supplicant *wpa_s = eloop_ctx;
	struct wpa_ssid *ssid;
...
#ifdef ANDROID
	printf("[ZWX] wpa_supplicant_scan : wpa_s->scanning  = %d \n",wpa_s->scanning);
	if (wpa_s->scanning) {
	//if (0) {
...
void wpa_supplicant_req_scan(struct wpa_supplicant *wpa_s, int sec, int usec)
{
	 ...
	 */
    printf("[ZWX] wpa_supplicant_req_scan \n");
...
{
    printf("[ZWX] wpa_supplicant_req_sched_scan \n");
	struct wpa_driver_scan_params params;
	struct wpa_driver_scan_params *scan_params;
	enum wpa_states prev_state;
	...
	if (wpa_s->autoscan_params != NULL) {
		scan_params = wpa_s->autoscan_params;
		printf("[ZWX] wpa_supplicant_req_sched_scan : goto scan \n");
		goto scan;
	}
	...
	if (!ssid || !wpa_s->prev_sched_ssid) {
		...
		if (wpa_s->conf->sched_scan_interval) {
		    printf("[ZWX] wpa_supplicant_req_sched_scan : sched_scan_interval = wpa_s->conf->sched_scan_interval \n");
		    wpa_s->sched_scan_interval =
				wpa_s->conf->sched_scan_interval;
		}
		if (wpa_s->sched_scan_interval == 0) {
			wpa_s->sched_scan_interval = 10;
			printf("[ZWX] wpa_supplicant_req_sched_scan : sched_scan_interval default 10 \n");
		}
...
scan:
	...
	printf("[ZWX] wpa_supplicant_req_sched_scan : wpa_supplicant_start_sched_scan \n");
...
{
    printf("[ZWX] wpa_supplicant_update_scan_results \n");
	struct wpa_scan_results *scan_res;
