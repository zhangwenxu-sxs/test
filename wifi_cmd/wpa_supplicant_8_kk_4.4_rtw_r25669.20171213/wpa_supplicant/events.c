...
static int _wpa_supplicant_event_scan_results(struct wpa_supplicant *wpa_s,
					      ...
					      int own_request)
{
    printf("[ZWX] _wpa_supplicant_event_scan_results \n");
					      ...
					      int new_scan)
{
	...
	if (selected) {
		...
		if (ssid) {
			...
			printf("[ZWX] wpas_select_network_from_last_scan \n");
...
#ifndef CONFIG_NO_STDOUT_DEBUG
{
	...
	case EVENT_SCAN_RESULTS:
	    printf("[ZWX] wpa_supplicant_event : EVENT_SCAN_RESULTS \n");
		wpa_supplicant_event_scan_results(wpa_s, data);
		printf("[ZWX] wpa_s->wpa_state = %d \n", wpa_s->wpa_state);
		if (wpa_s->wpa_state != WPA_AUTHENTICATING &&
		    wpa_s->wpa_state != WPA_ASSOCIATING) {
		    printf("[ZWX] wpa_s->wpa_state = %d \n", wpa_s->wpa_state);
	...
	case EVENT_SCHED_SCAN_STOPPED:
		...
		if (wpa_s->sched_scan_timed_out) {
		    printf("[ZWX] EVENT_SCHED_SCAN_STOPPED\n");
