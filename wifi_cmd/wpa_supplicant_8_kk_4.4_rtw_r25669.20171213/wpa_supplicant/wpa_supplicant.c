...
{
    printf("------------------------wpa_supplicant_start_bgscan-----------------------------\n");
...
void wpa_supplicant_associate(struct wpa_supplicant *wpa_s,
			      struct wpa_bss *bss, struct wpa_ssid *ssid)
{
    printf("---------------wpa_supplicant_associate----------------\n");
	u8 wpa_ie[200];
	size_t wpa_ie_len;
				     ...
				     struct wpa_interface *iface)
{
	...
	if (iface->confname) {
#ifdef CONFIG_BACKEND_FILE      //配置项是否来源于文件
		...
		wpa_s->conf = wpa_config_read(wpa_s->confname, NULL);   //解析运行时配置文件
		if (wpa_s->conf == NULL) {
    ...
    //将 wpa_interface 中的 ifname 复制到 wpa_supplicant 的 ifname 变量中
...
next_driver:
    ...
    //设置 wpa_s->wpa 指向一个 wpa_sm 对象，下面这两个函数用于设置 wpa_sm 中的一些成员变量
	wpa_sm_set_ifname(wpa_s->wpa, wpa_s->ifname,
			  wpa_s->bridge_ifname[0] ? wpa_s->bridge_ifname :
			  NULL);
	wpa_sm_set_fast_reauth(wpa_s->wpa, wpa_s->conf->fast_reauth);

    //如果配置文件 wpa_supplicant.conf 设置了变量dot11RSNAConfigPMKLifetime、dot11RSNAConfigPMKReauthThreshold和dot11RSNAConfigSATimeout ，则使用配置文件中的值来替换 wpa_sm 中的默认值。
	...
	wpa_s->gas = gas_query_init(wpa_s); //GAS 相关
	if (wpa_s->gas == NULL) {
