        ...
        unsigned char *buf, const size_t n_buf, const unsigned int user, const int flag)
{
    ...
    __SetCsLow(CS_LOW_DELAY);
    SpiWriteByte(cmd);

    if ( SPIC_ADDR_VALID & flag) {
        addr1 = (addr >> 16) & 0xff;
        addr2 = (addr >> 8) & 0xff;
        addr3 = addr & 0xff;
    ...
    if (flag & SPIC_READ_BYTES) {
        if (buf == 0) {
            fpga_log("NULL pointer\n");
            retval = -1;
            goto exit;
        }

        for (count = 0; count < n_buf; count++) {
            SpiReadByte(&buf[count]);
        }
        retval = count;
        ...
        for (count = 0; count < n_buf; count++) {
            SpiWriteByte(buf[count]);
        }

        retval = count;
    }

    __SetCsHigh(CS_HIGH_DELAY * 2);
