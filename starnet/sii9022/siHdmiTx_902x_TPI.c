...
void TXHAL_InitPostReset (void) 
{
	...
	TPI_TRACE_PRINT((">>TXHAL_InitPostReset()\n"));
...
//------------------------------------------------------------------------------
extern void sii9022_gpio_reset(void);
...
byte EnableInterrupts(byte Interrupt_Pattern)
{
	...
	Tmp = ReadByteTPI(TPI_DE_CTRL);
	//EnableEmbeddedSync();
	printk("TPI_DE_CTRL- = %x\n",Tmp);
	return TRUE;
}
...
byte Parse861Extensions (byte NumOfExtensions)
{
    ...
    do
    {
        ...
        {
			TPI_TRACE_PRINT(("read next 128 bytes of EDID ROM\n"));
...
void SetFormat (byte *Data)
{
    ...
    printk("SetFormat:Data[0] = %x,Data[1] = %x\n",Data[0],Data[1]);
...
void siHdmiTx_TPI_Poll (void)
{
	...
	if (g_sys.txPowerState == TX_POWER_STATE_D0)
	{
		...
		if (((InterruptStatus & RX_SENSE_STATE) >> 3) != g_sys.dsRxPoweredUp)
		{
			...
			DelayMS(500); // Delay for metastability protection and to help filter out connection bouncing
		}
        #endif
