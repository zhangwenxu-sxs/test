...
//                    LT8522EX_VrtopSelect();
    ...
    if (curr_lt8522ex->event != 0) {
        ...
        if(curr_lt8522ex->event & EVENT_HDMI_IN_PLUG_OUT) {
            switch_set_state(&curr_lt8522ex->sw_hdmi_in, 0);
        }
        if(curr_lt8522ex->event & EVENT_VGA_IN_PLUG_IN) {
            switch_set_state(&curr_lt8522ex->sw_vga_in, 1);
        }
        if(curr_lt8522ex->event & EVENT_VGA_IN_PLUG_OUT) {
            switch_set_state(&curr_lt8522ex->sw_vga_in, 0);
        }
        if(curr_lt8522ex->event & EVENT_HDMI_OUT_PLUG_IN) {
            switch_set_state(&curr_lt8522ex->sw_hdmi_out, 1);
        }
        if(curr_lt8522ex->event & EVENT_HDMI_OUT_PLUG_OUT) {
            switch_set_state(&curr_lt8522ex->sw_hdmi_out, 0);
        }
        if(curr_lt8522ex->event & EVENT_VGA_OUT_PLUG_IN) {
            switch_set_state(&curr_lt8522ex->sw_vga_out, 1);
        }
        if(curr_lt8522ex->event & EVENT_VGA_OUT_PLUG_OUT) {
...
static void lt8522ex_request_gpio(lt8522ex_t* lt8522ex)
{
    ...
    if (lt8522ex->hdmi_out_hpd_pin != INVALID_GPIO)
        gpio_request(lt8522ex->hdmi_out_hpd_pin, NULL);
...
static void lt8522ex_free_gpio(lt8522ex_t* lt8522ex)
{
    ...
    if (lt8522ex->hdmi_out_hpd_pin != INVALID_GPIO)
        gpio_free(lt8522ex->hdmi_out_hpd_pin);
    if (lt8522ex->vga_in_detect_pin != INVALID_GPIO)
...
static int lt8522ex_probe(struct platform_device* pdev)
{
    ...
    for_each_child_of_node(pdev->dev.of_node, child) {
        ...
        //获取检测HDMI OUT接口HPD状态的GPIO
        ret = of_property_read_u32(child, "hdmi_out_hpd_pin", &lt8522exs[lt8522ex_cnt].hdmi_out_hpd_pin);
        if (ret != 0) {
            lt8522ex_log("get hdmi_out_hpd_pin failed \n");
        }

        if (ret != 0 || lt8522exs[lt8522ex_cnt].hdmi_in_hpd_pin == 0xff) {
            lt8522exs[lt8522ex_cnt].hdmi_out_hpd_pin = INVALID_GPIO;
        }
