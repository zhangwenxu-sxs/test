...
char TxHpdChangeDetect(void)
{
    ...
    //没接GPIO的时候用读寄存器的方式判断HDMI OUT接口的连接状态
    if (curr_lt8522ex->hdmi_out_hpd_pin == INVALID_GPIO) {
        curr_client = curr_lt8522ex->client_addr2;
        newtxhpdstatus = HDMI_ReadI2C_Byte(0x7A) & 0x80 ? !0 : 0;
    } else {
        gpio_direction_input(curr_lt8522ex->hdmi_out_hpd_pin);
        newtxhpdstatus = !gpio_get_value(curr_lt8522ex->hdmi_out_hpd_pin);
    }
